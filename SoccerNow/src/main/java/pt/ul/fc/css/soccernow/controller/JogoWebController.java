...
public class JogoWebController {
  ...
  public JogoWebController(
      JogoService jogoService, TeamService teamService, RefereeService refereeService) {
    this.jogoService = jogoService;
    this.teamService = teamService;
    this.refereeService = refereeService;
  }

  /** GET /web/jogos Muestra la lista completa de juegos (sin filtros). */
  @GetMapping
  ...
  /**
   * GET /web/jogos/create Muestra formulario vacío para crear un juego. Pone en el modelo la lista
   * de equipos y árbitros.
   */
  @GetMapping("/create")
  ...
  /** POST /web/jogos/save Recibe el JogoDTO del formulario y crea el juego. */
  @PostMapping("/save")
  public String saveJogo(@ModelAttribute("jogoDTO") JogoDTO dto, Model model) {
    try {
      ...
      if (dto.getPrimaryRefereeId() != null) {
        Referee primary =
            refereeService
                .getRefereeById(dto.getPrimaryRefereeId())
  ...
  /**
   * GET /web/jogos/edit/{id} Muestra formulario con datos del juego para editar (excepto
   * resultado).
   */
  @GetMapping("/edit/{id}")
  ...
  /** POST /web/jogos/update/{id} Actualiza un juego existente (solo campos mutables). */
  @PostMapping("/update/{id}")
  public String updateJogo(
      @PathVariable Long id, @ModelAttribute("jogoDTO") JogoDTO dto, Model model) {
    try {
      Jogo existing =
          jogoService
              .obterJogo(id)
  ...
  /** GET /web/jogos/delete/{id} Elimina un juego si no tiene resultado. Redirige a la lista. */
  ...
  /**
   * GET /web/jogos/filters Lista juegos aplicando filtros: status (played, pending, cancelled),
   * location, minGoals, timeSlot. Parámetros opcionales: status, location, minGoals, timeSlot.
   */
  @GetMapping("/filters")
